...
const MOCK_CLOSING = [
  { id:1, cat:'Grocery',  fav:true,  name:'Ajinomoto Salt', convQty:1000, closingStock:'0 GM',   units:['GM','Milligram'] },
  { id:2, cat:'Grocery',  fav:true,  name:'Ajinomoto Salt', convQty:null, closingStock:'0 Pkts',  units:['Pkts']          },
  { id:3, cat:'Disposal', fav:true,  name:'Bag L',          convQty:100,  closingStock:'0 Pkts',  units:['Pkts','NOS']    },
  { id:4, cat:'Disposal', fav:true,  name:'Bag M',          convQty:100,  closingStock:'0 Pkts',  units:['Pkts','NOS']    },
  { id:5, cat:'Disposal', fav:true,  name:'Bag S',          convQty:100,  closingStock:'0 Pkts',  units:['Pkts','NOS']    },
  { id:6, cat:'Disposal', fav:true,  name:'Black Contener', convQty:50,   closingStock:'0 Pkts',  units:['Pkts','NOS']    },
  { id:7, cat:'Grocery',  fav:true,  name:'Black Pepper',   convQty:1000, closingStock:'0 GM',    units:['GM','Milligram'] },
  { id:8, cat:'Backed',   fav:true,  name:'Breads',         convQty:14,   closingStock:'0 Pkts',  units:['Pkts','Piece']  },
]
...
// Add Stock form
function AddStockForm({ type, onSave, onCancel }) {
...
function ClosingStockPage() {
  ...
  if (showAdd) return <AddStockForm type="Closing" onSave={()=>setShowAdd(false)} onCancel={()=>setShowAdd(false)}/>

  const filtered = MOCK_CLOSING.filter(r=>
    (!rawSearch || r.name.toLowerCase().includes(rawSearch.toLowerCase())) &&
    (!category || r.cat===category)
  )
  ...
  return (
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18 }}>
        <h1 style={{ fontSize:20, fontWeight:800, color:'#1a1a2e', margin:0 }}>Closing Stock</h1>
        <div style={{ display:'flex', gap:10 }}>
          <button onClick={()=>setShowAdd(true)} style={{
            ...BTN_RED, display:'flex', alignItems:'center', gap:6,
            boxShadow:'0 2px 8px rgba(229,62,62,.25)' }}>
      ...
      <div style={{ background:'#fff', border:'1px solid #e8eaed', borderRadius:10,
        ...
        <table style={{ width:'100%', borderCollapse:'collapse' }}>
          <thead>
            <tr>
              <th style={TH}>Category</th>
              <th style={TH}>Raw Material</th>
              <th style={TH}>Closing Stock<br/><span style={{fontWeight:400,color:'#aaa',fontSize:10}}>(5 May 2026)</span></th>
              <th style={{ ...TH, textAlign:'center' }} colSpan={2}>Update Your Closing Stock</th>
              <th style={TH}>Comments</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((item,i)=>(
              <tr key={item.id} style={{ background:i%2===0?'#fff':'#fdfdfd' }}
                ...
                <td style={{ ...TD, fontWeight:600, color:'#333' }}>{item.closingStock}</td>
                {item.units.map((unit,ui)=>(
                  <td key={ui} style={TD}>
                    <div style={{ display:'flex', alignItems:'center', gap:4 }}>
                      <input type="number"
        ...
        <div style={{ display:'flex', justifyContent:'flex-end', padding:'12px 16px',
          borderTop:'1px solid #f0f0f0', background:'#fafafa' }}>
          <button onClick={()=>toast.success('Closing stock saved!')}
            style={{ ...BTN_RED, boxShadow:'0 2px 8px rgba(229,62,62,.25)' }}>
            Save
          </button>
        </div>
...
export { AvailableStockPage, ClosingStockPage }
